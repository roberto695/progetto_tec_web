<?php
// ============================================================
// pages/registrazione.php – Registrazione nuovo utente
// ============================================================
session_start();
...
// Valori da ripopolare in caso di errore
$val = [
    'nome'    => '',
    'cognome' => '',
    'cf'      => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ripopola campi (senza password per sicurezza)
    $val = [
    ...
    if ($nome === '') {
        ...
        $errori['nome'] = 'Il nome non può superare i 50 caratteri.';
    }

    if ($cognome === '') {
        $errori['cognome'] = 'Il cognome è obbligatorio.';
    } elseif (mb_strlen($cognome) > 50) {
        $errori['cognome'] = 'Il cognome non può superare i 50 caratteri.';
    }

    if ($cf === '') {
        $errori['cf'] = 'Il Codice Fiscale è obbligatorio.';
    } elseif (!preg_match('/^[A-Z0-9]{16}$/', $cf)) {
        $errori['cf'] = 'Il Codice Fiscale deve essere di esattamente 16 caratteri alfanumerici.';
    }

    if ($email === '') {
        $errori['email'] = 'L\'indirizzo email è obbligatorio.';
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $errori['email'] = 'Inserisci un indirizzo email valido (es. nome@dominio.it).';
    }

    if ($password === '') {
        $errori['password'] = 'La password è obbligatoria.';
    } elseif (mb_strlen($password) < 6) {
        $errori['password'] = 'La password deve contenere almeno 6 caratteri.';
    }
    ...
    if (empty($errori)) {
        ...
        $stmt->execute([$email]);
        if ($stmt->fetch()) {
            $errori['email'] = 'Questa email è già associata a un account.';
        }
    }
    ...
    if (empty($errori)) {
        $stmt = $pdo->prepare(
            'INSERT INTO persona (cf, nome, cognome, telefono, email, password, ruolo)
             VALUES (?, ?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([
            $cf,
            ...
            $email,
            $password,   // in chiaro (requisito progetto)
            'user'
        ]);

        $success = true;
        // Resetta i valori dopo successo
        $val = ['nome'=>'','cognome'=>'','cf'=>'','email'=>'','telefono'=>''];
    }
}

// Helper: classe CSS campo errato
function err_class(string $field, array $errori): string {
    return isset($errori[$field]) ? ' form-input--error' : '';
}

function err_aria(string $field, array $errori): string {
    return isset($errori[$field]) ? ' aria-describedby="' . $field . '-error" aria-invalid="true"' : '';
}
?>
<!DOCTYPE html>
<html lang="it">
    ...
    <div class="auth-page" style="align-items: flex-start; padding-top: var(--space-32);">
        <div class="auth-card" style="max-width: 560px;">
            ...
            <p class="auth-card__subtitle">
                Registrati per prenotare i tuoi esami del sangue online.
            </p>
            ...
            <div class="alert alert--success" role="status" aria-live="polite">
                ...
                <div>
                    ...
                    <a href="login.php">Accedi ora</a> per prenotare il tuo esame.
                </div>
            </div>
            <?php endif; ?>
            ...
            >
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input
                            ...
                            required
                            aria-required="true"
                            <?= err_aria('nome', $errori) ?>
                        >
                        <?php if (isset($errori['nome'])): ?>
                            <span class="form-error" id="nome-error" role="alert">
                                <?= htmlspecialchars($errori['nome'], ENT_QUOTES, 'UTF-8') ?>
                            </span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="form-label" for="cognome">
                            Cognome <span class="required" aria-label="obbligatorio">*</span>
                        </label>
                        <input
                            type="text"
                            id="cognome"
                            ...
                            required
                            aria-required="true"
                            <?= err_aria('cognome', $errori) ?>
                        >
                        <?php if (isset($errori['cognome'])): ?>
                            <span class="form-error" id="cognome-error" role="alert">
                                <?= htmlspecialchars($errori['cognome'], ENT_QUOTES, 'UTF-8') ?>
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <label class="form-label" for="cf">
                        Codice Fiscale <span class="required" aria-label="obbligatorio">*</span>
                    </label>
                    <input
                        type="text"
                        id="cf"
                ...
                <div class="form-group">
                    <label class="form-label" for="email">
                        Indirizzo email <span class="required" aria-label="obbligatorio">*</span>
                    </label>
                    <input
                        type="email"
                        id="email"
                ...
                <div class="form-group">
                    <label class="form-label" for="password">
                        Password <span class="required" aria-label="obbligatoria">*</span>
                    </label>
                ...
                <!-- Privacy -->
                <div class="form-group">
                    <label class="form-check<?= isset($errori['privacy']) ? ' form-input--error' : '' ?>">
                        <input
                            type="checkbox"
                            name="privacy"
                            id="privacy"
                        ...
                        <span>
                            ...
                            <a href="#" aria-label="Leggi la privacy policy (si apre in questa pagina)">
                                Privacy Policy
                            </a>
                    ...
                    <?php if (isset($errori['privacy'])): ?>
                        <span class="form-error" id="privacy-error" role="alert">
                            <?= htmlspecialchars($errori['privacy'], ENT_QUOTES, 'UTF-8') ?>
                        </span>
                    <?php endif; ?>
                </div>
